...
async def send_photo(user_id: int, photo, disable_notification: bool = False) -> bool:
    try:
        await dp.bot.send_photo(user_id, photo, disable_notification=disable_notification)
    except exceptions.BotBlocked:
        log.error(f"Target [ID:{user_id}]: blocked by user")
    except exceptions.ChatNotFound:
        log.error(f"Target [ID:{user_id}]: invalid user ID")
...
async def broadcaster(post=None, photo=None, video=None, file=None) -> int:
    count = 0
    if post is not None:
    ...
    if photo is not None:
        for user_id in await get_users():
            if await send_photo(user_id, photo=photo):
                count += 1
            await asyncio.sleep(.05)  # 20 messages per second (Limit: 30 messages per second)
